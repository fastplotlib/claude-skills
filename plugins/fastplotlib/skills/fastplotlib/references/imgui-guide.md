...
[imgui](https://github.com/pthom/imgui_bundle) UIs are rendered directly onto the same canvas as the `Figure`, so
...
see `imgui.WindowFlags_`. The examples gallery has a menu bar example.
...
`imgui.WindowFlags_`. `is_open` tells you whether the popup is currently open.
...
fastplotlib does not wrap imgui, you call `imgui_bundle` directly, so any imgui element can be used. `references/imgui-elements.md`
documents each element as it exists in `imgui_bundle`, with its signature, its arguments and its
flags.
